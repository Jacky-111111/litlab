...
## Migration: collection sharing v1 (2026-04-28)

Enables real sharing on `public.collections` via three visibility modes
(`private` / `selected` / `public`). Anonymous + selected read access is
enforced by the FastAPI backend using the service role; owner-only RLS
continues to protect direct client access.

> **Run order:** this migration ships as **two** files under
> `backend/migrations/` because PostgreSQL requires a new enum value to be
> committed before it can be referenced. Run them in order in the Supabase
> SQL Editor:
>
> 1. `0002_collection_sharing__step1_enum.sql` — adds the `'selected'`
>    enum value.
> 2. `0003_collection_sharing__step2_tables.sql` — everything else
>    (user_profiles changes, legacy `'link'` row migration, and the
>    `collection_shared_users` table + RLS).
>
> Both files are idempotent.

The combined SQL below is for reference only (what the two step files do
together):
...
-- 2) Extend collection_visibility enum and migrate legacy 'link' rows.
alter type public.collection_visibility add value if not exists 'selected';
update public.collections set visibility = 'selected' where visibility = 'link';
